...
`r rmarkdown::metadata$title`: Effects of increasing temperature on sea star growth and feeding rate
...
Juvenile *P. ochraceus* will be reared in the lab, at temperatures ranging from 5 - 21 &deg;C. We will use twenty-four tanks, 246L in volume, with recirculating water to house seastars. Two seastars will be placed inside each tank, contained in their own tupperware with mesh sides and tops to ensure water flow, for a total of 48 seastars. Relative growth of the 2 seastars inside a single tank will be averaged, thus tank is the independent unit in this design. 
...
Each of the 48 seastars used in this study will be randomly assigned to tanks. We can run a simple linear regression to ensure no bias for initial mass for assigned temperature.  
...
We plan to sample 48 individuals, this size complies with our lab space constraints of 24 available tanks. Specimens will be collected in January from Jericho Beach, Vancouver. 
...
We have simulated data based on the previous study of which we are replicating, including review of summary statistics. This was to ensure that, using the same methods, this experiment was replicate. 
...
48 individuals will be sample due to equipment restrictions.As there will be 2 individuals in each tank we will use the average between them for growth and feeding rate to avoid pseudo-replication.  
...
We will be manipulating the water temperature in the 24 tanks from a range of 5 - 21 &deg;C.
...
We will measure growth rate (change in wet mass / initial mass). Initial wet mass will be taken of each individual by removing them from the tank, lightly patting them dry and then weighing them to the nearest 0.1g. A final wet mass will be taken at the end of the experiment, and then again after acclimated all individuals back to 13 &deg;C. This step is to ensure that difference in water temperature did not affect water retention. 
...
The mean between both individuals of a single tank will be taken for growth and feeding rate to avoid pseudo replication as then are subjected to the same manipulated condition. 
...
A simple linear regression will be used for both growth rate under increasing temperature and feeding rate under increasing temperature. Here the independent variable for both analyses is temperature, which is a manipulated range. Growth rate and feeding rate are the dependent variables.
